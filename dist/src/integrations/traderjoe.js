...
async function discoverTraderJoeWallets() {
    ...
    try {
        ...
        const sJoeHolders = await (0, seeds_js_1.getTokenHolders)(exports.TRADERJOE_CONTRACTS.SJOE_TOKEN, 'avalanche');
        ...
        const joeHolders = await (0, seeds_js_1.getTokenHolders)(exports.TRADERJOE_CONTRACTS.JOE_TOKEN, 'avalanche');
        ...
        for (const pairAddress of majorPairs) {
            ...
            const pairLPs = await (0, seeds_js_1.getLiquidityProviders)(pairAddress, 'avalanche');
            discoveredWallets.push(...pairLPs);
        }
        // 4. Remove duplicates and validate addresses
        const uniqueWallets = discoveredWallets.filter((wallet, index, arr) => arr.findIndex(w => w.value === wallet.value) === index).filter(wallet => (0, seeds_js_1.isValidAddress)(wallet.value, wallet.chain));
...
async function scanTraderJoeRewards(wallets) {
    ...
    try {
        ...
        for (let i = 0; i < Math.min(wallets.length, 5); i++) {
            ...
            const mockReward = {
                ...
                wallet,
                protocol: 'traderjoe',
                token: { value: exports.TRADERJOE_CONTRACTS.USDC, chain: 'avalanche' },
                amountWei: ethers_1.ethers.parseUnits((Math.random() * 50 + 10).toFixed(6), 6).toString(), // 10-60 USDC
...
async function buildTraderJoeBundles(rewards) {
